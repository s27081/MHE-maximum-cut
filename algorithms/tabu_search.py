...
def tabu_search(graph, max_iterations, tabu_list_size, tabu_history_size):
    partition = random_partition(graph)
    best_partition = partition.copy()
    cut = max_cut_goal_function(graph, best_partition, True)
    tabu_list = []
    tabu_history_list = []
    ...
    for i in range(max_iterations):
        ...
        if best_tabu is None:
            history_workspace = return_to_working_point(graph, tabu_history_list, tabu_list)
...
def return_to_working_point(graph, tabu_history_list, tabu_list):
